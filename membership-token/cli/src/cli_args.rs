...
/// CLI arguments.
#[derive(Parser, Debug)]
#[clap(name = "mpl-membership-token-cli")]
#[clap(about = "CLI utility for mpl-membership-token program")]
#[clap(version, author)]
pub struct CliArgs {
    /// RPC endpoint.
...
pub enum Commands {
    /// Obtain `SellingResource` account from `mpl_membership_token` program.
    GetSellingResource {
        #[clap(short, value_name = "STRING")]
        account: String,
    },
    /// Obtain `Store` account from `mpl_membership_token` program.
    GetStore {
        #[clap(short, value_name = "STRING")]
        account: String,
    },
    /// Obtain `Market` account from `mpl_membership_token` program.
    GetMarket {
        #[clap(short, value_name = "STRING")]
        account: String,
    },
    /// Obtain `TradeHistory` account from `mpl_membership_token` program.
    GetTradeHistory {
        #[clap(short, value_name = "STRING")]
        account: String,
    },
    /// Perform `CreateStore` instruction of `mpl_membership_token` program.
    CreateStore {
    ...
    /// Perform `Buy` instruction of `mpl_membership_token` program.
    Buy {
